...
module.exports={
    transfermoney:async function(data){
        ...
        let toexist = await accservice.checkAccountExists(data.fromno)
       ...
       let fromid =  await accservice.getUser(data.fromno);
        ...
        return await transervice.transfermoney(data.fromno,data.tono,fromid,toid,data.amount).then(val=>{
            return {msg:"money transferred successfully",success:true}
        })
    },
